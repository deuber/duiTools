...
@echo SVNserve config differs per OS and SVN version, review Polarion Deployment and Maintenance Guide >> %logDir%/svn.log
...
@echo If "This is very likely to create a memory leak" see DPP-207930 corrupts polarion-data >> %logDir%/concerns.log
...
/s /p /c:"java.lang.OutOfMemoryError" ^
/s /p /c:"This is very likely to create a memory leak" ^
